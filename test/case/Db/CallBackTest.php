...
class CallBackTest extends DatabaseTest
{
	...
	{
		$cb = $this->callback->register('after_construct',function($mode) { echo $mode; });
		$this->assert_has_callback('after_construct');
	}
	...
	{
	    //exact the same, stubbed it to point to that same code. otherwise method can be dropped
	    $this->test_register();
	}
	...
	public function test_register_via_static_with_invalid_definition()
	{
		...
		$s = new $class_name();
		$o = new ActiveRecord\CallBack($class_name);
	}
