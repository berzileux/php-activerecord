...
class RelationshipTest extends DatabaseTest
{
	...
	{
	    $this->expectException(ActiveRecord\DatabaseException::class, "host_id cannot be null");
	...
	public function test_eager_loading_has_many_array_of_includes()
	{
		...
		foreach ($assocs as $assoc)
		{
 		...
 		$q = "WHERE author_id IN(?,?)";
 		$this->assert_sql_has($q ,ActiveRecord\Table::load('Author')->last_sql);
 		$this->assert_sql_has($q ,ActiveRecord\Table::load('Book')->last_sql);
 		$this->assert_sql_has($q ,ActiveRecord\Table::load('AwesomePerson')->last_sql);
	}
